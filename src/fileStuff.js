...
export const stringifyUserCSV = (users) => {
    return "CID,name,key (DEC),PIN,comment,\n" + stringify(users.map(user => [user.cid, user.name, user.uid, user.pincode]));
};
...
export const logUser = (logFile, success, action, door, user) => {
    ...
    fs.appendFile(logFile, `${new Date().toISOString()} [${action}] success: ${success}, cid:"${user.cid}" name:"${user.name}", uid:"${user.uid}", pin:"${user.pincode}"\n`, (err) => {
